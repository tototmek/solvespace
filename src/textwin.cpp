...
public:
    ...
    std::string tooltip;
    std::string iconName;
    std::shared_ptr<Pixmap> showIcon;
    std::shared_ptr<Pixmap> hideIcon;
    ...
    void Draw(UiCanvas *uiCanvas, int x, int y, bool asHovered) override {
        if(showIcon == NULL) {
            showIcon = LoadPng("icons/text-window/" + iconName + ".png");
            hideIcon = LoadPng("icons/text-window/" + iconName + "-hidden.png");
        }
        ...
        if(*(variable)) {
            uiCanvas->DrawPixmap(showIcon, x, y - 24, 2);
        } else {
            uiCanvas->DrawPixmap(hideIcon, x, y - 24, 2);
        }
    }
...
const TextWindow::Color TextWindow::fgColors[] = {
    {'d', RGBi(140, 140, 140)},                           // Default   : white
    {'l', RGBi(84, 138, 97)},                             // links     : blue
    {'t', RGBi(200, 200, 200)},                           // tree/text : yellow
    {'h', RGBi(255, 255, 0)},   {'s', RGBi(88, 145, 60)}, // Ok        : green
    {'m', RGBi(200, 200, 200)}, {'r', RGBi(0, 255, 0)},   // Reverse   : black
    {'x', RGBi(100, 0, 100)},                             // Error     : red
    {'i', RGBi(145, 77, 60)},                             // Info      : cyan
    {'g', RGBi(255, 255, 0)},                             // Disabled  : gray
    {'b', RGBi(115, 115, 115)}, {0, RGBi(21, 21, 21)}};
/** Background color codes. */
const TextWindow::Color TextWindow::bgColors[] = {{'d', RGBi(21, 21, 21)}, // Default   : black
                                                  {'t', RGBi(42, 42, 42)},
                                                  {'a', RGBi(28, 28, 28)}, // Alternate : dark gray
                                                  {'r', RGBi(140, 140, 140)}, // Reverse   : white
                                                  {0, RGBi(0, 0, 0)}};
...
void TextWindow::DrawOrHitTestIcons(UiCanvas *uiCanvas, TextWindow::DrawOrHitHow how, double mx,
    ...
    if(how == PAINT) {
        ...
        uiCanvas->DrawRect(0, (int)width, top, bot,
                           /*fillColor=*/{28, 28, 28, 255}, /*outlineColor=*/{});
    }
...
void TextWindow::Paint() {
    ...
    Lighting lighting        = {};
    lighting.backgroundColor = RGBf(0.12, 0.12, 0.12);
